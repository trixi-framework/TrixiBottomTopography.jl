...
Topo_Cart_orth  = CartData(xyz_grid(-0.5:0.1:0.5,-0.9:0.1:0.9,0))
...
df_xyz = DataFrame(x = vec(Topo_Cart_orth.x.val[:,:,1]), 
                   y = vec(Topo_Cart_orth.y.val[:,:,1]), 
                   z = vec(Topo_Cart_orth.z.val[:,:,1]))

# Schreibe die Datei ohne Header und mit Leerzeichen als Delimiter
open("test.xyz", "w") do file
