...
style throughout TrixiBottomTopography.jl, even at the expense that not everyone can be happy with every
...
## Conventions
The following lists a few coding conventions for TrixiBottomTopography.jl. Note that in addition to these
...
source code of TrixiBottomTopography.jl, which will also enforce *some* of the [Conventions](@ref) listed
...
[`.JuliaFormatter.toml`](https://github.com/trixi-framework/TrixiBottomTopography.jl/blob/main/.JuliaFormatter.toml)
file that holds the configuration options we use for JuliaFormatter.jl.

Note that we expect all contributions to TrixiBottomTopography.jl to be formatted with JuliaFormatter.jl
...
```
You can then recursively format the core Julia files in the TrixiBottomTopography.jl repo by executing
```shell
julia -e 'using JuliaFormatter; format(".")'
```
from inside the TrixiBottomTopography.jl repository. For convenience, there is also a script you can
...
```shell
utils/trixibt-format.jl
```
You can get more information about using the convenience script by running it with the
`--help`/`-h` flag.
There is an additional script to format individual files that can be run from the terminal shell
```shell
utils/trixibt-format-file.jl path/to/file.jl
```
...
runs over the whole TrixiBottomTopography.jl directory.
